    ...
    return (
      <Link to={`/dashboard/news/${judulArtikel}`} className='max-w-[23%] max-h-[50vh] bg-white shadow-md p-2 rounded-xl flex flex-col cursor-default'>
          ...
          <article className='flex gap-x-2 justify-end items-center mt-auto'>
              <ThumbsUp className={`border-[2px] border-blue/60 stroke-blue ${ favorite.includes(_id) ? 'border-transparent bg-blue/80 stroke-white' : '' } hover:border-transparent hover:bg-blue/60 hover:stroke-white duration-200 ease-in-out p-1 rounded-md w-6 h-6`} />
              <Pin className={`border-[2px] border-pink-400 stroke-pink-400 ${ bookmark.includes(_id) ? 'border-transparent bg-pink-400 stroke-white' : '' } hover:border-transparent hover:bg-pink-400 hover:stroke-white duration-200 ease-in-out p-1 rounded-md w-6 h-6`} />
          </article>
          </div>
      </Link>
    )
